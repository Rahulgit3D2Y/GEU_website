  ...
  if(isset($submit))
{
 ...
 $studentsessionquery=mysqli_query($con,"SELECT * FROM `session`  WHERE `fyear`='$dateyear'");
	    ...
	    $rnotice=mysqli_query($con,"SELECT * FROM `syllabus` WHERE `syllabus_session`='$studentsessionyearrecord' AND `syllabus_courseid` ='$inputCourseName' AND `syllabus_semester` ='$inputSemester' AND `syllabus_status` = '$status'");
...
$profilephotouploadname="syllabus_".date('dmY')."".time(); 
    ...
    $uploadfolder = 'upload/syllabus/';
    move_uploaded_file($tmpphotoname, $uploadfolder.$basename);

        mysqli_query($con,"INSERT INTO `syllabus`(`syllabus_session`, `syllabus_courseid`, `syllabus_semester`, `syllabus_fileupload`, `syllabus_status`, `syllabus_createdby`, `syllabus_createdtime`) VALUES ('$studentsessionyearrecord','$inputCourseName','$inputSemester','$basename','$status','$log','$date')") or die(mysqli_error("<script language='javascript'>alert('Error Please Contact To Administrator');window.location='$Currentwebsiteurl'</script>"));
echo "<script language='javascript'>alert('syllabus For $studentsessionyearrecord Added Successfully')</script>"; 
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-12">
                                <div class="card shadow-lg border-0 rounded-lg mt-1">
                                    <div class="card-header"><h3 class="text-center font-weight-light my-0">Add Syllabus</h3></div>
                                    <div class="card-body">
                            ...
                            <div class="card-body">
                                ...
                                <table id="datatablesSimpl">
                                    ...
                                    <tbody>
                                        ...
                                        $count = 0;
                                        $noticequery=mysqli_query($con,"SELECT * FROM `syllabus` WHERE `syllabus_status`=\"Active\" AND `syllabus_session`='$activesession_record' ORDER BY `syllabus`.`syllabus_id` DESC");
                                        while($noticequeryresult=mysqli_fetch_assoc($noticequery))
    {
        $count+=1;
$id=$noticequeryresult['syllabus_id'];
$inputCourseNameType=$noticequeryresult['syllabus_courseid'];
$created_user_id=$noticequeryresult['syllabus_createdby'];
...
$user_login_name=$user_name_resultrow12['first_name']." ".$user_name_resultrow12['middle_name']." ".$user_name_resultrow12['last_name'];
                    ...
                    $course_name=$resultcoursequery['course_acroym'];
                     $imagepath= $noticequeryresult['syllabus_fileupload'];
                     $filenames="Syllabus_".$noticequeryresult['syllabus_session']."_".$course_name."_".$noticequeryresult['syllabus_semester'];
                      $filename=str_replace('.', '', $filenames);
        if($imagepath)
        {
                $imageencode = "upload/syllabus/$imagepath";
        }
        else
        {
          $imageencode = "upload/syllabus/error.pdf";
        }
...
$imageencodesrc = 'data:'.mime_content_type($imageencode).';base64,'.$imageData;
                                        ...
                                        <tr>        
                                            ...
                                            <td><?php echo $noticequeryresult['syllabus_session']; ?></td>
                                            <td><?php echo $course_name; ?></td>
                                            <td><?php echo $noticequeryresult['syllabus_semester']; ?></td>
                                            <td><a href="<?php echo $imageencodesrc; ?>" download="<?php echo $filename; ?>"><?php echo $filename; ?></a></td>
                                            <td><?php echo $user_login_name; ?></td>
                                            <td>
                                              <a href="syllabus_delete.php?syllabus_id=<?php echo urlencode(base64_encode($id)); ?>"><span class='fas fa-trash'></span></a>
                                            </td>
                                        </tr>
