...
<section>
    <form class="d-flex p-3 tcolor"  method="POST" action="syllabus.php">
...
if(isset($search))
{
    ...
    $InputSectionType=$_POST['InputSection'];
     ...
     $rstudent=mysqli_query($con,"SELECT * FROM `syllabus` WHERE `syllabus_session`='$inputsessionyearType' AND `syllabus_courseid` ='$inputCourseNameType' AND `syllabus_semester` ='$InputSemesterType' AND `syllabus_status` = 'Active'");
if (mysqli_num_rows($rstudent)<1)
{
     $studenterrormsg="Syllabus is not uploaded yet";

} 
 else
 { 
$result12=mysqli_query($con,"SELECT * FROM `syllabus` WHERE `syllabus_session`='$inputsessionyearType' AND `syllabus_courseid` ='$inputCourseNameType' AND `syllabus_semester` ='$InputSemesterType'  AND `syllabus_status` = 'Active'");
...
$coursequery=mysqli_query($con,"SELECT * FROM `course_detail` WHERE `course_id2`='$inputCourseNameType'");
            ...
            $studenterrormsg="";
            $imagepath= $row1['syllabus_fileupload'];
            $filenames= "Syllabus_".$inputsessionyearType."_".$course_name."_".$InputSemesterType;
            $filename=str_replace('.', '', $filenames);
        if($imagepath)
        {
                $imageencode = "admin/upload/syllabus/$imagepath";
        }
        else
        {
          $imageencode = "admin/upload/syllabus/error.pdf";
        }
