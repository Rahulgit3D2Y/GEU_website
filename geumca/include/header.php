...
<meta name="twitter:image:src">
  ...
  <title> Department Of Computer Applications | GEU </title>
      ...
      <nav class="navbar navbar-light ">
        ...
        <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
          <ul class="navbar-nav ml-auto">
            <?php 
$sqlforheader=mysqli_query($con,"SELECT * FROM `header` WHERE `header_status`='Active' AND `header_display`='Active' AND `header_pagemore`='No' ORDER BY `header_order` ASC");
                                        while($sqlforheaderresult=mysqli_fetch_assoc($sqlforheader))
    {
?>
            <li class="nav-item">
              <a class="nav-link text-light" href="<?php echo $sqlforheaderresult['header_url']; ?>"><?php echo $sqlforheaderresult['header_name']; ?></a>
            </li>
          <?php } ?>
